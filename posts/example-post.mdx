---
title: 测试页
description: "有些东西一直存在。 根据物理学，永远不可能有真正的物理虚无——尽管有时存在类似于虚无，例如真空（最小可能能量的状态）（Phys.org）。 创造一个没有量子的空间。"
date: 
---

有些东西一直存在。 根据物理学，永远不可能有真正的物理虚无——尽管有时存在类似于虚无，例如真空（最小可能能量的状态）（Phys.org）。 创造一个没有量子涨落的空间需要巨大的能量，如果涨落被冲走，那空间里就会有剩余的能量，再加上不稳定的环境（1veritasium）。 即使在计算机上，删除的数据实际上也不会被丢弃，而是被覆盖。
